...
class scheduler:
    ...
    def load(self, program_description):
        ...
        if len(tmp) != 2:
            print
            'Bad description (%s): Must be number <x:y>' % program_description
            print
        ...
        while self.get_num_active() > 0:
            ...
            for pid in range(len(self.proc_info)):
                if pid == self.curr_proc and instruction_to_execute != '':
                    print
                    '%10s' % ('RUN:' + instruction_to_execute),
                else:
                    print
                    '%10s' % (self.proc_info[pid][PROC_STATE]),
